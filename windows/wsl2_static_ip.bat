@echo off
wsl -d Ubuntu18.04 -u root ip addr del $(ip addr show eth0 ^| grep 'inet\b' ^| awk '{print $2}' ^| head -n 1) dev eth0
wsl -d Ubuntu18.04 -u root ip addr add 192.168.50.2/24 broadcast 192.168.50.255 dev eth0
wsl -d Ubuntu18.04 -u root ip route add 0.0.0.0/0 via 192.168.50.1 dev eth0
